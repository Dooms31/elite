...
get_player_stats_individual <- function(..., progress = TRUE, strip_redundancy = TRUE, other = "") {
  ...
  .get_player_stats_individual <- function(player_url, name, ...) {
    ...
    if (is.na(player_url)) {
      ...
      vitals <- page %>%
        ...
        mutate_all(~na_if(., "-")) %>%
        mutate_all(~na_if(., "")) %>%
        select(-c(feet_tall, inches_tall, youth_team))
      ...
      if ("GAA" %in% colnames(skater_or_goalie)) {
        ...
        player_statistics <- page %>%
          ...
          select(team_, league_, captaincy_, season_, season_short_, games_played_, goals_, assists_, points_, penalty_minutes_, plus_minus_, goals_against_average_, save_percentage_, games_played_playoffs_, goals_playoffs_, assists_playoffs_, points_playoffs_, penalty_minutes_playoffs_, plus_minus_playoffs_, goals_against_average_playoffs_, save_percentage_playoffs_) %>% 
  ...
  if ("pick_number" %in% colnames(mydata)) {
    ...
    mydata <- mydata %>%
      ...
      select(draft_league, draft_year, pick_number, round, draft_team, name, position, birth_place, birth_country, birthday, height, weight, player_url, name_, position_, player_url_, player_statistics) %>%
      mutate_at(vars(c(draft_league, draft_year, draft_team, name, position, birth_place, birth_country, birthday, name_, position_, player_url_, player_url)), as.character) %>%
      mutate_at(vars(-c(draft_league, draft_year, draft_team, name, position, birth_place, birth_country, birthday, name_, position_, player_url_, player_url, player_statistics)), as.numeric)
    ...
    mydata <- mydata %>%
      ...
      select(name, team, league, position, birth_place, birth_country, birthday, height, weight, season, season_short, games_played, goals, assists, points, penalty_minutes, plus_minus, games_played_playoffs, goals_playoffs, assists_playoffs, points_playoffs, penalty_minutes_playoffs, plus_minus_playoffs, player_url, team_url, name_, position_, player_url_, player_statistics) %>%
      mutate_at(vars(c(name, team, league, position, birth_place, birth_country, birthday, season, player_url, team_url, name_, position_, player_url_)), as.character) %>%
      mutate_at(vars(-c(name, team, league, position, birth_place, birth_country, birthday, season, player_url, team_url, name_, position_, player_url_, player_statistics)), as.numeric)
    ...
    mydata <- mydata %>%
      select(name, position = position_, birth_place, birth_country, birthday, height, weight, player_url, name_, player_url_, player_statistics) %>%
      mutate_at(vars(c(name, position, birth_place, birth_country, birthday, player_url, name_, player_url_)), as.character) %>%
      mutate_at(vars(-c(name, position, birth_place, birth_country, birthday, player_url, name_, player_url_, player_statistics)), as.numeric)
