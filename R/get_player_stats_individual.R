...
get_player_stats_individual <- function(..., progress = TRUE, strip_redundancy = TRUE, other = "") {
  ...
  if ("pick_number" %in% colnames(mydata)) {
    ...
    mydata <- mydata %>%
      ...
      select(draft_league, draft_year, pick_number, round, draft_team, name, position, shot_handedness, birth_place, birth_country, birthday, height, weight, player_url, name_, position_, player_url_, player_statistics) %>%
